...
```typescript TypeScript (EventStream)
...
events.on('afStatus', (data) => {
  console.log('AF:', data.state);
});
```

```typescript TypeScript (CameraManager)
// CameraManager handles SSE internally — use manager events instead
manager.on('camera-ready', ({ cameraId, camera }) => {
  console.log(`${camera.model} connected`);
});

manager.on('camera-disconnected', ({ cameraId, error }) => {
  console.log(`Disconnected: ${error ?? 'clean'}`);
});
```
</CodeGroup>
