...
<Note>
  The Alpha Camera REST API is a community-built developer tool and is **not officially supported by Sony**. Only the [Camera Remote SDK](/sdk/camera-remote-sdk) (C++/C#) is an official Sony product.
</Note>
...
The Alpha Camera REST API is an HTTP + SSE server built on Sony's native [Camera Remote SDK](/sdk/camera-remote-sdk). It translates RESTful requests into native SDK calls, enabling camera control from any language or tool that can make HTTP requests.
...
To run the server itself: `npm install -g @alpha-sdk/api && camera-server start`. Or embed it in your Node app via the [`ServerManager`](/web-api/server-package) class.

## Quick Start

<Card title="Alpha Camera REST API Quickstart" href="/quickstarts/http-api">
