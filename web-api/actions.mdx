...
<ParamField path="method" type="POST">`POST /api/cameras/{cameraId}/actions/movie-rec`</ParamField>

Start or stop video recording. This is a toggle action — call once to start recording, call again to stop. Uses the SDK's hold-style command (Down to start, Up to stop).
...
```typescript TypeScript
await cam('D10F60149B0C').toggleMovieRecording();
```

```json Response
{
  "success": true,
  "message": "Movie recording toggled"
}
```
</CodeGroup>
