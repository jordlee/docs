...
Both servers are compatible with Anthropic's LLMs (claude.ai, Claude Mobile, Claude Desktop, and Claude Code) as well as other AI-powered IDEs like Cursor. You can install one or both.
...
<Tabs>
  ...
  <Tab title="Claude Code">
    ...
    **Or approve specific tools only:**
    ...
    | `approvedTools` not working           | Verify the server name in your MCP config matches the prefix in `approvedTools` (e.g. `CameraRemoteSDK`, `CameraHelp`).                |
