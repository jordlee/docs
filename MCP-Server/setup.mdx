...
Sony provides two MCP servers for AI-assisted development:

| Server | Description | URL |
|--------|-------------|-----|
| **AI SDK Assistant** | Camera Remote SDK documentation, API references, and code examples | `https://sdk-rag-agent-production.up.railway.app/mcp` |
| **AI Camera Assistant** | Camera help guides, settings, and feature documentation for 24 Sony camera models | `https://camera-rag-agent-production.up.railway.app/mcp` |

Both servers are compatible with Anthropic's LLMs (claude.ai, Claude Mobile, Claude Desktop, and Claude Code) as well as other AI-powered IDEs like Cursor.
...
## AI Camera Assistant Setup

The AI Camera Assistant is a separate MCP server that provides help guide documentation for **24 Sony camera models** with **55,000+** indexed help guide entries. It covers camera settings, features, shooting modes, and troubleshooting across Alpha, FX, ZV, and other Sony camera lines.

<Info>
  The Camera Assistant uses the same setup process as the SDK Assistant above — just with a different server URL and name. You can run both servers simultaneously.
</Info>

<Tabs>
  <Tab title="Claude">
    Follow the same steps as the [SDK Assistant setup above](#configuration-steps), but use:
  ...
  <Tab title="Claude Code">
    ...
    <Tabs>
      ...
      <Tab title="Windows (PowerShell or CMD)">
        ```bash
        claude mcp add CameraAssistant --scope user -- cmd /c npx mcp-remote https://camera-rag-agent-production.up.railway.app/mcp
        ```
      </Tab>
    </Tabs>
    ...
    Add alongside your existing `CRSDK` server in your MCP config file:
    ...
    Auto-approve Camera Assistant tools:

    ```json
    {
      "approvedTools": ["mcp__CameraAssistant"]
    }
    ```
  </Tab>
  <Tab title="Cursor">
    Follow the same steps as the [SDK Assistant setup above](#quick-setup-settings-ui), but use:

    - **Name:** `CameraAssistant`
    - **Command:** `npx mcp-remote https://camera-rag-agent-production.up.railway.app/mcp`

    Or add to your `.cursor/mcp.json` alongside the SDK server:
...
The AI Camera Assistant has help guides indexed for the following cameras:
