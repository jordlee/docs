...
<Tabs>
  <Tab title="Claude">
    ...
    4. **Add System Prompt (Optional)**
       ...
       By default, Claude will use the included tool descriptions to guide which search method it uses.

       However, you can also add a system prompt to refine how Claude behaves.

       A system prompt is a set of detailed instructions with examples and guidelines on how the LLM should use tools, format responses, and clarify ambiguities. Using a system prompt can improve response quality and relevance.

       [Configure the system prompt.](../tips#using-the-system-prompt)

    <Note>
      We recommend trying the AI SDK Assistant without the system prompt to start. If you notice Claude making false assumptions or skipping over important SDK content, you can add the system prompt.
    </Note>
  ...
  <Tab title="Claude Code">
    ...
    **Remove a server:**
    ...
    ### **Add Subagents and System Prompt (Optional)**

    In Claude Code, the default chat window uses a single instance of Claude (main agent).

    You can create up to 3 specialized instances of Claude (subagents) to work on specific tasks and define different roles (via the system prompt).

    Use this feature when you want to have multiple tasks running in parallel to improve efficiency and retain task-specific context (e.g. one subagent researches SDK documentation and writes code, one runs tests, while another writes documentation).

    [Configure the system prompt.](../tips#using-the-system-prompt)

    <Note>
      We recommend using the AI SDK Assistant with your main agent when you need Claude to maintain context awareness across multiple tasks and files.
    </Note>
    ### Tool Permissions
...
5. **Configure the System Prompt (Recommended)**

   By default, ChatGPT tends to revert to web searches instead of using the AI SDK Assistant, due to its poor MCP support.

   However, you can add a system prompt to instruct ChatGPT to always use the AI SDK Assistant.
