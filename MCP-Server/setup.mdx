...
The Kando MCP server is compatible with Claude (web, mobile, desktop, and CLI), and ChatGPT (web). These platforms use what are colloquially termed "connectors" to access MCP servers directly from the existing UI.
...
<Tabs>
  <Tab title="Claude">
    ...
    2. **Configure Tools**
       ...
       Kando enables Claude to access SDK information through "Tools." These are specialized search methods and filters that influence what type of content is retrieved when you ask a question.
       ...
       To configure Kando's tools:
       - Navigate to **Settings → Connectors**
       - Next to the Kando connector, click the **Configure** button
       - You'll now be able to see all available tools Kando offers
    ...
    To start using Kando's tools:

    1. Open a **new chat window**
    2. Click the **search and tools** icon button in the chat box
    3. You should see the Kando connector at the top of the list
    4. Toggle the connector **on** to enable it
    5. You can also toggle individual tools directly in the chat window

    ### Removing Kando

    If you decide you no longer want to keep Kando as a connector:

    - Go to **Settings → Connectors**
    - Click the **More** icon button next to the Kando connector
    - Click **Remove** to permanently remove the connector (you'll need to repeat the setup process to add Kando again)
