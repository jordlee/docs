---
title: "Tips"
description: "Best practices for using Iris to learn and implement the Camera Remote SDK"
icon: "lightbulb"
---

## General Prompting Tips

Get better results from Iris by following these best practices when asking questions about the Camera Remote SDK.
...
Iris has access to working C++ and C# code examples from the SDK. Don't hesitate to request them.
...
## When and How to Use Iris

### Automatic Tool Selection

Iris uses **13+ specialized search tools** to find the best answers for your questions. Your AI assistant automatically chooses the right tool based on what you're asking.

<Note>
  You don't need to specify which tool to use—just ask your question naturally. Iris's AI will determine whether to search documentation, code examples, API references, compatibility tables, or error codes.
</Note>

### Available Search Capabilities

Iris can search across:
...
Iris currently searches **SDK V2.00.00** by default. If you need information from V1.14.00, just specify it in your question.
...
### When Iris Searches Automatically

Your AI assistant will automatically use Iris when you ask questions about:
...
<Warning>
  **For Claude Code users**: Unlike Claude web, Claude Code does not support granular tool permissions. All Iris tools are available once the MCP server is added. The only control is the initial approval when using project-scoped servers.
</Warning>
...
Ask Iris to show you how the official sample apps implement features:
...
- **Tool Reference**: See [Tools](/MCP-Server/tools) for detailed information about Iris's search capabilities
- **Troubleshooting**: Check [Troubleshooting](/MCP-Server/troubleshooting) for common setup and usage issues
- **Setup Guide**: Review [Setup](/MCP-Server/setup) if you need to reconfigure Iris

<Tip>
  The more specific and detailed your questions, the better Iris can help you learn and implement the Camera Remote SDK!
</Tip>
