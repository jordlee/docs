...
| ------------------------------- | ---------------------------------- | -------------- | ------------------------------------------------------------------ | --------------------------------------------------------- | ----------------------------------------------- | -------------------------------- |
| **Claude (Web/Mobile/Desktop)** | Pro, Max, or Enterprise (\$20+/mo) | 200K tokens    | ✅ Granular UI controls<br />Per-tool "ask" vs "unsupervised"       | Good, but no codebase access.                             | Cloud-based<br />Data retained per policy       | Most users, maximum control      |
| **Claude Code (CLI)**           | Included with paid Claude account  | 200K tokens    | ✅ Granular file-based config<br />Approve server or specific tools | Excellent, access to your codebase for context.           | Local configs<br />No additional data retention | Developers, automation workflows |
    ...
    **Best for**: Users with existing ChatGPT subscriptions looking for an alternative
    ...
    <Warning>
      **Not Recommended Unless You Already Have a ChatGPT Subscription**
      ...
      Due to OpenAI's unofficial support for MCP servers, we do not recommend using ChatGPT with the AI SDK Assistant unless you already have an active subscription. The tool may not function correctly due to:
      - **Poor code generation**: Produces pseudo-code only, not production-ready SDK implementations
      - **Limited MCP support**: Unofficial implementation with reduced functionality
      - **Unreliable tool usage**: May prefer web search over AI SDK Assistant tools
      - **Basic tool control**: Cannot selectively enable/disable specific search tools

      **We strongly recommend using Claude (web, mobile, desktop, or CLI) for SDK development work.**
    </Warning>
